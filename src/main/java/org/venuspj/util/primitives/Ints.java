...
/**
 * This class provides utility methods for working with int values.
 */
public final class Ints {
  ...
  /**
   * Converts a long value to an integer, performing a checked cast to prevent overflow. If the
   * casted value is not equal to the original value, an exception is thrown.
   *
   * @param value the long value to be converted to an integer
   * @return the integer value after a successful cast
   * @throws IllegalArgumentException if the casted value is not equal to the original value,
   ...
   * {@code concat(new int[] {a, b}, new int[] {}, new int[] {c}} returns the array
   * {@code {a, b, c}}.
   *
   ...
   * {@code Collections.reverse(Ints.asList(array))}, but is likely to be more efficient.
   */
   ...
   * {@code Collections.reverse(Ints.asList(array).subList(fromIndex, toIndex))}, but is likely to
   * be more efficient.
   *
